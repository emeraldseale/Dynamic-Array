...
struct student* create_student(char* name, int id, float gpa) {
  ...
  char* heap_name = malloc(sizeof(char) * strlen(name)); 
...
struct dynarray* create_student_array(int num_students, char** names, int* ids,
  ...
  for (i = 0; i < num_students; i++) {
     dynarray_insert(student_array, i, create_student(names[i], ids[i], gpas[i]));
  }
...
struct student* find_max_gpa(struct dynarray* students) {
  ...
  int i;
  struct student* s = ((struct student *)dynarray_get(students, 0));

  for(i = 0; i < ((dynarray_size(students)) - 1); i++) {
    if(s->gpa < ((struct student *)dynarray_get(students, (i + 1)))->gpa) {
      s = ((struct student *)dynarray_get(students, (i + 1)));
    }
  }
  ...
  for (i = 0; i < (num - 1); i++) {
    for (j = 0; j < (num - i - 1); j++) {
      if(((struct student *)dynarray_get(students, (j)))->gpa < ((struct student *)dynarray_get(students, (j + 1)))->gpa) {
        temp = ((struct student *)dynarray_get(students, (j)));
        dynarray_set(students, j, ((struct student *)dynarray_get(students, (j + 1))));
